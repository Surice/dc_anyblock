...
export async function info(msg: Message, content: string[]): Promise<void> {
  ...
  let embed = new MessageEmbed()
    ...
    .setDescription(
      `Username Check: ${toggle(
        gc.usernameCheckEnabled
      )} \nBlock Scamlinks: ${toggle(
        gc.scamlinkCheckEnabled
      )} \nBlock links: ${toggle(
        gc.globalLinkBlockEnabled
      )} \nBlock unauthorized everyone: ${toggle(
        gc.blockUnauthorizedEveryoneEnabled
      )}`
    )
    .addFields([
      {
        name: "Link blocked channels",
        value: gc.linkBlockChannel?.join(", ") || "- none- ",
        inline: true,
      },
      {
        name: "Link allowed channels",
        value: gc.linkAllowedChannel?.join(", ") || "- none -",
        inline: true,
      },
      {
        name: "\u200b",
      ...
      {
        name: "Link whitelist",
        value: gc.linkWhitelist?.join(", ") || "- none -",
        inline: true,
      },
      {
        name: "Link blacklist",
        value: gc.linkBlacklist?.join(", ") || "- none -",
        inline: true,
      },
    ]);
